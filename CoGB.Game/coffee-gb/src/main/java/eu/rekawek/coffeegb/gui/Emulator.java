...
public class Emulator {
    ...
    public Emulator(String[] args, Properties properties) throws IOException {
        ...
        hubConnection = HubConnectionBuilder.create(/*"http://localhost:5000/hubGame"*/ env.get("GBSignalR"))
                .build();
        ...
        if (options.isHeadless()) {
            ...
            sound = null; //new AudioSystemSoundOutput();
            display = new SwingDisplay(SCALE, hubConnection);
            controller = new SwingController(properties, hubConnection);
            gameboy = new Gameboy(options, rom, display, controller, SoundOutput.NULL_OUTPUT, serialEndpoint, console);
        }
